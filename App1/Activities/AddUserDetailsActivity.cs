       ...
       // SetContentView(Resource.Layout.activity_add_user_details_);
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            btnRegister.Click += async delegate
            {
                ...
                user.user_Password      = editPassword.Text;
                user.user_PhoneNumber   = editPNumber.Text;
                user.user_Address       = editAddress.Text;
                user.user_Country       = editCountry.Text;

                HttpClient client = new HttpClient();
                string url = "https://localhost:44366/api/userinfors?firstName={user.FirstName}&lastname={user.LastName}&address={user.Address}&password{user.Password}&phonenumber{user.PhoneNumber}&country{user.Country}&username{user.Email}";
